...
function OverviewTab({trays,putWall,batches,events,messages:initMessages,onSendMessage,onBatchControl}){
  ...
  const [panelOrder,setPanelOrder]=useState(["kpis","messages","machines","grid","fleet"]);
  const [drag,setDrag]=useState({dragging:null,over:null});
  const [showSlackCfg,setShowSlackCfg]=useState(false);
  const [draft,setDraft]=useState({});

  const handleIncoming=useCallback((msg)=>{
    onSendMessage && onSendMessage(msg.text, msg); // bubble up for event log
    setMessages ? setMessages(prev=>[msg,...prev].slice(0,50)) : null;
  },[]);
  ...
  // QR = deep-link to the configured Slack channel URL, else generic slack.com
  ...
  const handleDrop=(targetId)=>{
    ...
    setPanelOrder(prev=>{
      const next=[...prev];
      const fromIdx=next.indexOf(drag.dragging);
      const toIdx=next.indexOf(targetId);
      next.splice(fromIdx,1);
      next.splice(toIdx,0,drag.dragging);
      return next;
    });
  ...
  const PANELS={
    ...
    messages:(
      <Card style={{borderLeft:`4px solid #4A154B`}}>
        {/* Header row with Slack connection badge */}
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          ...
          <button onClick={()=>{setDraft({...slack.cfg});setShowSlackCfg(s=>!s);}}
            style={{fontSize:11,padding:"5px 12px",background:showSlackCfg?"#4A154B":T.bg,border:`1px solid ${showSlackCfg?"#611f69":T.border}`,borderRadius:6,color:showSlackCfg?"#E8A9F4":T.textMuted,cursor:"pointer",fontFamily:mono,fontWeight:700,transition:"all 0.2s"}}>
        ...
        {showSlackCfg&&(
          <div style={{marginBottom:14,padding:16,background:"#1A0A1E",border:"1px solid #611f69",borderRadius:10}}>
            ...
            <div style={{fontSize:11,color:T.textDim,marginBottom:12,lineHeight:1.6}}>
              1. In Slack: <strong style={{color:T.text}}>Apps → Incoming Webhooks → Add New</strong><br/>
              2. Pick your channel (e.g. <code style={{color:"#E8A9F4",background:"#2A0A2E",padding:"1px 5px",borderRadius:3}}>#lab-assistant</code>)<br/>
              3. Copy the Webhook URL and paste below
            </div>
            {[
              {key:"webhook",     label:"Outgoing Webhook URL",     ph:"https://hooks.slack.com/services/T.../B.../...", type:"url",  help:"Apps → Incoming Webhooks → Add New"},
              {key:"channel",     label:"Channel name (no #)",       ph:"lab-assistant",                                type:"text", help:"e.g. lab-assistant or lens-lab"},
              {key:"channelUrl",  label:"Channel URL (for QR code)", ph:"https://yourco.slack.com/archives/C...",       type:"url",  help:"Right-click channel → Copy Link"},
              {key:"channelId",   label:"Channel ID (for incoming)", ph:"C05AB12XYZ",                                  type:"text", help:"Last part of the channel URL above"},
              {key:"proxyUrl",    label:"Local proxy URL (incoming)",ph:"http://localhost:3001/slack/messages",         type:"url",  help:"Run slack-proxy.js on your server — provides real-time incoming messages"},
            ].map(f=>(
              <div key={f.key} style={{marginBottom:10}}>
                ...
                />
                {f.help&&<div style={{fontSize:8,color:"#9B5CA8",fontFamily:mono,marginTop:2}}>ⓘ {f.help}</div>}
              </div>
            ))}
            <div style={{display:"flex",gap:8,marginTop:4}}>
              <button onClick={()=>{slack.save(draft);setShowSlackCfg(false);}}
                style={{flex:1,padding:"9px",background:"#4A154B",border:"1px solid #611f69",borderRadius:6,color:"#E8A9F4",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:mono}}>
                💾 SAVE & CONNECT
              </button>
              {slack.cfg.webhook&&(
                <button onClick={()=>slack.post("🔗 Lab_Assistant connected successfully!")}
                  style={{padding:"9px 14px",background:"transparent",border:"1px solid #611f69",borderRadius:6,color:"#E8A9F4",cursor:"pointer",fontFamily:mono,fontSize:11,fontWeight:700}}>
                  {slack.status==="sending"?"…":slack.status==="ok"?"✓ OK":slack.status==="err"?"✗ ERR":"TEST"}
                </button>
              )}
              {slack.cfg.webhook&&(
                <button onClick={()=>{slack.save({});setShowSlackCfg(false);}}
                  style={{padding:"9px 14px",background:"transparent",border:`1px solid ${T.border}`,borderRadius:6,color:T.textDim,cursor:"pointer",fontFamily:mono,fontSize:11}}>
                  Disconnect
                </button>
              )}
            </div>
        ...
        {/* Message feed + input + QR */}
        <div style={{display:"flex",gap:20}}>
          <div style={{flex:1}}>
            <div style={{maxHeight:150,overflowY:"auto",marginBottom:10}}>
              {messages.map(m=>(
                <div key={m.id} style={{display:"flex",gap:12,padding:"6px 0",borderBottom:`1px solid ${T.border}`}}>
                  <div style={{width:6,height:6,borderRadius:"50%",marginTop:5,flexShrink:0,background:m.priority==="high"?T.red:T.green,boxShadow:m.priority==="high"?`0 0 6px ${T.red}`:"none"}}/>
                  <div style={{flex:1}}>
                    <div style={{fontSize:12,color:T.text}}>
                      {m.source==="slack"&&<span style={{fontSize:9,background:"#4A154B",color:"#E8A9F4",borderRadius:3,padding:"1px 5px",fontFamily:mono,fontWeight:700,marginRight:5}}>↓ SLACK</span>}
                      <strong style={{color:m.source==="slack"?"#9ECBF9":"#E8A9F4"}}>{m.from}:</strong> {m.text}
                    </div>
                    <div style={{fontSize:9,color:T.textDim,fontFamily:mono,marginTop:1}}>{m.time.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</div>
                  </div>
                  {m.priority==="high"&&<Pill color={T.red}>HOT</Pill>}
                </div>
              ))}
            </div>
            <div style={{display:"flex",gap:8}}>
              <input value={msgInput}
                ...
                placeholder={isConnected?`Message #${slack.cfg.channel||"lab-assistant"} + dashboard…`:"Type a message (connect Slack to broadcast)…"}
              ...
              <button onClick={handleSend}
                style={{background:sendBg,border:"none",borderRadius:6,padding:"8px 18px",color:slack.status?"#fff":"#fff",fontWeight:800,fontSize:11,cursor:"pointer",fontFamily:mono,transition:"background 0.3s",minWidth:72}}>
                {sendLabel}
              </button>
            </div>
            {isConnected&&<div style={{fontSize:9,color:T.textDim,fontFamily:mono,marginTop:4}}>↑ Messages broadcast to Slack in real-time</div>}
          </div>

          {/* QR Code — links to Slack channel */}
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:6,padding:"10px 14px",background:T.bg,borderRadius:8,border:`1px solid ${isConnected?"#611f69":T.border}`,transition:"border-color 0.3s"}}>
            <QRCode data={qrData} size={92}/>
            <div style={{fontSize:8,color:isConnected?"#E8A9F4":T.textDim,fontFamily:mono,textAlign:"center",fontWeight:700}}>
              {isConnected?`SCAN → SLACK\n#${slack.cfg.channel||"lab-assistant"}`:"SCAN → SLACK"}
            </div>
          </div>
        </div>
      </Card>
        ...
        <Card>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:6}}>
            {putWall.map((s,i)=>{
              ...
              return(
                <div key={i} style={{background:bg,border:`1px solid ${border}`,borderRadius:6,padding:"7px 5px",textAlign:"center",position:"relative",minHeight:60,display:"flex",flexDirection:"column",justifyContent:"center"}}>
      ...
      <Card style={{padding:20}}>
              ...
              <div key={deptKey} style={{display:"flex",flexDirection:"column"}}>
                <div style={{textAlign:"center",paddingBottom:8,marginBottom:8,borderBottom:`2px solid ${dept.color}`}}>
                  ...
                  <div style={{marginTop:2}}>
                    <Pill color={rushInDept>0?T.red:T.textDim} bg={rushInDept>0?`${T.red}20`:`${T.textDim}15`}>
                      {rushInDept} RUSH
                    </Pill>
                  </div>
                </div>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:12,paddingTop:10,borderTop:`1px solid ${T.border}`,justifyContent:"center"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:4,padding:"2px 6px"}}>
            <div style={{width:7,height:7,borderRadius:"50%",background:"#475569"}}/><span style={{fontSize:10,color:"#475569",fontFamily:mono}}>Unassigned</span>
          </div>
        </div>
      </Card>
    ),
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}} onDragEnd={handleDragEnd}>
      {panelOrder.map(id=>(
        <DraggablePanel key={id} id={id} dragState={drag} onDragStart={handleDragStart} onDragOver={handleDragOver} onDrop={handleDrop}>
          {PANELS[id]}
        </DraggablePanel>
      ))}
    </div>
...
When generating reports: use ## for main sections, ### for subsections, - for bullet points, **bold** for key metrics. Be specific with actual numbers from the data. Flag anything below 90% pass rate as a concern. Be concise but comprehensive.`;
  ...
  const sendMessage=async(text)=>{
    ...
    try{
      ...
      const msgIdx=newMessages.length; // index of the new assistant message
      setMessages(prev=>[...prev,{role:"assistant",content:reply,isReport:willBeReport,prompt:userText,msgIdx}]);
    }catch(e){
      ...
      {subView==="fleet"?(
        ...
        <div style={{display:"grid",gridTemplateColumns:"400px 1fr",gap:20}}>
          <Card style={{borderTop:`3px solid ${T.green}`}}>
            ...
            {/* Mode toggle */}
            {(()=>{
              const [bindMode,setBindMode]=useState("pos"); // "pos" | "tray"
              return null; // managed below in state
            })()}
