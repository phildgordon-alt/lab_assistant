...
//
// Phil 2026-05-14: hardened against THE SAME EPIPE cascade that caused
// 3,253 launchd restarts (confirmed in server-errors.log). The 2026-04-30
// fix wrapped writes in try/catch, but stream.write() is async — EPIPE
// arrives later on the stream's 'error' event. No listener → bubbles to
// uncaughtException → re-enters this handler → writes stderr again →
// loops forever. Real fix: listen for 'error' on stdout/stderr, latch a
// broken flag, and STOP writing to that stream once it's confirmed dead.
// The file write keeps working.
...
function enrichJob(j, xmlIndex) {
  ...
  return {
    ...
    frameUpc: xml?.frameUpc || null,
    frameStyle: xml?.frameStyle || null,
    lensType: xml?.lensType || null,
    lensStyle: xml?.lensStyle || null,
    lensMaterial: xml?.lensMat || null,
    lensColor: xml?.lensColor || null,
    lensOpcR: xml?.lensOpc || null,
    lensOpcL: xml?.lensOpcL || null,
    department: xml?.department || null,
    jobType: xml?.jobType || null,
    isHko: xml?.isHko ? 1 : 0,
    entryTime: xml?.entryTime || null,
    rxNum: xml?.rxNum || '',
    isRush: xml?.rush === 'Y',
...
- Any notes about timing, upcoming capacity constraints, or efficiency tips`;
  ...
  if (req.method==='POST' && url.pathname==='/api/som/thresholds/reload') {
    return json(res, { ok: true, thresholds: som.reloadThresholds() });
  }
...
<\/script>
  ...
  // GET /api/flow/production-history?days=14
  if (req.method==='GET' && url.pathname==='/api/flow/production-history') {
    const days = parseInt(url.searchParams.get('days') || '14');
    try {
      return json(res, flowAgent.getProductionHistory(days));
    } catch (e) {
      console.error('[Flow] production-history error:', e.message);
      return json(res, { error: e.message }, 500);
    }
  }
